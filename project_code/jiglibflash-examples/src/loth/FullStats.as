...
public class FullStats extends Sprite {
	 ...
	 */
	public function FullStats(width:int = 70, x:int = 0, y:int = 0, isMinimized:Boolean = false, isDraggable:Boolean = true, isMonitoring:Boolean = false, scale:Number = 1):void {
		//
		...
		style.setStyle('xmlData', {fontSize: '10px', fontFamily: 'Trebuchet MS', leading: '-1px'});
	...
	private function initGraph():void {
		if (!_isMinimized) {
			...
			//graph_BD.threshold(graph_BD, graph_BD.rect, new Point(), "<=", 0xFF222222, 0x0000000, 0xFFFFFFFF, true);
		}
	}
	...
	private function initMonitoring():void {
		...
		if (!_isMinimized) {
			if (_isMonitoring) {
				graphics.lineStyle(1, 0xFF0000, 1, true, LineScaleMode.NORMAL, CapsStyle.SQUARE, JointStyle.MITER);
				graphics.drawRect(BUTTON_XPOS, BUTTON_YPOS + BUTTON_MONITOR_GAP, BUTTON_SIZE, BUTTON_SIZE);
				graphics.drawRect(BUTTON_XPOS - 1, BUTTON_YPOS - 1 + BUTTON_MONITOR_GAP, BUTTON_SIZE + 2, BUTTON_SIZE + 2);
			} else {
				graphics.lineStyle(1, colors.monitorSeparator, 1, true, LineScaleMode.NORMAL, CapsStyle.SQUARE, JointStyle.MITER);
				graphics.drawRect(BUTTON_XPOS, BUTTON_YPOS + BUTTON_MONITOR_GAP, BUTTON_SIZE, BUTTON_SIZE);
			  	graphics.lineStyle(1, 0x000000, 1, true, LineScaleMode.NORMAL, CapsStyle.SQUARE, JointStyle.MITER);
				graphics.drawRect(BUTTON_XPOS - 1, BUTTON_YPOS - 1 + BUTTON_MONITOR_GAP, BUTTON_SIZE + 2, BUTTON_SIZE + 2);
			}
		}
		graphics.lineStyle();
	...
	private function handleFrameTick(event:Event):void {
		...
		if (_isMonitoring) {
			...
			monitorView_BD.threshold(monitorView_BD, monitorView_BD.rect, new Point(), "<=", 0xFF000000, 0x0000000, 0xFFFFFFFF, true);
